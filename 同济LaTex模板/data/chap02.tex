...
EKF 是一种卡尔曼滤波器，其通过对非线性系统的一阶线性近似从而进行状态估计。其算法 如算法 \ref{algEKF} 所示。其中，$\mathbf{P}$ 为协方差，表示真实状态和估计状态之间的误差。

\begin{algorithm}[htb] 
  \numberwithin{algorithm}{chapter}
  \caption{EKF}\label{algEKF}
  \begin{algorithmic}[1]
    \STATE 生成初始状态 $\hat{\mathbf{x}}_0^+$ 和 $\mathbf{P}_0^+$
    \FOR {$k=1,2,3,\cdots $}
    \STATE  $\hat{\mathbf{x}}_k^- = \mathbf{f}_{k-1}(\hat{\mathbf{x}}_{k-1}^+)$
    \STATE $\mathbf{F}_{k-1}  =  (\partial\mathbf{f}/ \partial\mathbf{x})|_{\mathbf{x}=\hat{\mathbf{x}}_{k-1}^+}$
    \STATE  $\mathbf{P}_k^-= \mathbf{F}_{k-1}\mathbf{P}_{k-1}^+\mathbf{F}_{k-1}^T+\mathbf{Q}_{k-1}$
    \STATE  $\mathbf{H}_{k}  =  (\partial\mathbf{h}/ \partial\mathbf{x})|_{\mathbf{x}=\hat{\mathbf{x}}_{k}^-}$
    \STATE  $\mathbf{K}_k = \mathbf{P}_k^- \mathbf{H}_k^T (\mathbf{H}_k \mathbf{P}_k^- \mathbf{H}_k^T + \mathbf{R}_{k})^{-1}$
    \STATE $\hat{\mathbf{x}}_k^+ = \hat{\mathbf{x}}_{k}^- + \mathbf{K}_k(\mathbf{z}_k - \mathbf{h}_k( \hat{\mathbf{x}}_k^-))$
    \STATE $\mathbf{P}_k^+ = (\mathbf{I}-\mathbf{K}_k \mathbf{H}_k) \mathbf{P}_k^-$
    \ENDFOR
  \end{algorithmic}  
\end{algorithm} 
