...
\RequirePackage[backend=biber,style=gb7714-2015,gbpub=false,isbn=false,doi=false]{biblatex}  
% 若需使用author-year格式的文献引用，可将style选择为：gb7714-2015ay
...
\def\tongji@title@font{%
  \iftongji@arialtitle\sffamily\else\heiti\fi}
...
\ctexset{%
  ...
  % 了，但必须在这里重新应用一次，用来保证章首页的页眉正确。
    ...
    format={\centering\bfseries\tongji@title@font\sanhao[1]},
...
%        \parbox[t][9cm][t]{\paperwidth-10cm}{
    ...
    {%
      ...
      \includegraphics[width=10cm]{tongji-whole-logo.eps}
    }
    \\[-.6cm] % logo
...
% 参考文献页眉处理
% % 这个\tjbibliographyheading暂时用来解决参考文献的第二页以后页面的页眉显示为正文章节页眉的问题。
 ...
 \let\tj@tmp@thebibliography\thebibliography
 \RenewDocumentEnvironment{thebibliography}{}
 {\tjbibliographyheading \tj@tmp@thebibliography}{\clearpage}
% 我发现不需要重新加入\tjbibliographyheading，只需要clearpage就行了。
%\let\tj@tmp@thebibliography\thebibliography
%\RenewDocumentEnvironment{thebibliography}{}
%{\tj@tmp@thebibliography}{\clearpage}
